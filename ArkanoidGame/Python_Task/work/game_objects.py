...
class Paddle:
    ...
    def _update_power_ups(self):
        ...
        if self.power_up_timers['laser'] > 0:
            ...
            if self.power_up_timers['laser'] <= 0 or self.laser_charges <= 0:
                self.has_laser = False
                self.laser_charges = 0
...
class PowerUp:
    PROPERTIES = {
        ...
        '2ball': {'color': (255, 105, 180), 'char': 'M', 'message': "X2 ball"},
        'multi': {'color': (255, 105, 180), 'char': 'M', 'message': "X2 ball"}  # alias for 2ball
    }
